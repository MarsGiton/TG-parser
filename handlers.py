...
Поиск Telegram-каналов и чатов, вперемешку с пометкой типа.
"""
...
from scraper import search_channels, TGChannel
...
def _format_channel(ch: TGChannel, index: int) -> str:
    lines = [f"{index}. {ch.type_emoji()} {hbold(ch.name)}"]
    lines.append(
...
def _format_results(channels: list[TGChannel], query: str) -> list[str]:
    if not channels:
        return [
    ...
    messages = []
    header = (
        f"🔍 Результаты по запросу <b>{query}</b>: {len(channels)}\n"
        f"Фильтр: от 1 000 подписчиков\n"
        f"📢 Канал · 💬 Чат\n"
        + "─" * 30
    )
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        "👋 <b>Channel Finder</b> — поиск Telegram-каналов и чатов для рекламы\n\n"
        "Напиши ключевое слово или используй /search:\n\n"
        "/search маркетинг\n"
        "/search таджвид\n"
        "/search фитнес Москва\n\n"
        "📢 Канал · 💬 Чат — помечено в результатах\n\n"
        f"Показывает до {MAX_RESULTS} результатов · от 1 000 подписчиков"
    )
...
async def cmd_help(message: Message):
    ...
    await message.answer(
        "📖 <b>Справка</b>\n\n"
        "/search [запрос] — поиск каналов и чатов\n"
        "/help — эта справка\n\n"
        "<b>Обозначения:</b>\n"
        "📢 — канал (только читают)\n"
...
async def _do_search(message: Message, query: str):
    wait_msg = await message.answer(f"🔍 Ищу каналы и чаты по запросу <b>{query}</b>...")
    try:
